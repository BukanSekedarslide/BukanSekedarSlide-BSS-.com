...
function orderText(form, est) {
  ...
  const labels = { vault: "Template Vault", custom: "Custom Express (Maks. 5 Slot/Hari)", beauty: "Beatifying Slide" };
  const payLabels = { dana: "Dana", qris: "QRIS", gopay: "GoPay" };
  const cart = loadCart();
  ...
  return [
    ...
    `Metode Pembayaran: ${payLabels[data.pembayaran] || data.pembayaran}`,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("cartCheckoutBtn").addEventListener("click", () => {
    ...
    const payMethod = $("cartPaySelect").value;
    const payLabels = { dana: "Dana", qris: "QRIS", gopay: "GoPay" };
    const total = items.reduce((s, x) => s + x.price, 0);
    const cartLines = items.map((x) => `- ${x.name} (${x.mode}, ${x.slides} slide) ${rupiah(x.price)} ${x.link ? '-> ' + x.link : ''}`).join("\n");
    
    const summaryText = [
      "Pesanan Template Vault BukanSekedarSlide (BSS)",
      `Metode Pembayaran: ${payLabels[payMethod]}`,
      "",
      "Daftar Item:",
      cartLines,
      "",
    ...
    navigator.clipboard?.writeText(summaryText).then(() => {
      toast("Ringkasan keranjang disalin! Membuka Lynk.id...");
    });
  ...
  ["layananSelect", "slideCount", "tenggatSelect", "pembayaranSelect"].forEach((id) => {
    if ($(id)) {
